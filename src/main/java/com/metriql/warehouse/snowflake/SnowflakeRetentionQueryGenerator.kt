...
class SnowflakeRetentionQueryGenerator : RetentionQueryGenerator {
    override fun generateSQL(auth: ProjectAuth, context: IQueryGeneratorContext, queryDSL: Retention, options: RetentionReportOptions): String {
        ...
        val template = if (options.approximate) throw MetriqlException(HttpResponseStatus.BAD_REQUEST) else expensive
        return jinja.render(
            template,
            mapOf(
    ...
    companion object {
        private val expensive = this::class.java.getResource("/sql/retention/warehouse/snowflake/expensive.jinja2").readText()
    }
}
