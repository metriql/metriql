...
val services = mapOf<ReportType, IAdHocService<out ServiceReportOptions>>(ReportType.SEGMENTATION to SegmentationService())
...
class QueryHttpService(val modelService: IModelService, val dataSource: DataSource, cacheService: ICacheService, val taskQueueService: TaskQueueService) : HttpService() {
    val reportService = ReportService(modelService, JinjaRendererService(), SqlQueryTaskGenerator(cacheService), services) {
        mapOf()
    }
