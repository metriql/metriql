...
abstract class TestSimpleFilter<C> {
    ...
    fun testAnyFiltersIsSet() {
        ...
        val query =
            "SELECT test_string FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter ORDER BY test_string"
    ...
    fun testAnyFiltersIsNotSet() {
        ...
        val generatedFilter = test.filter(table)
            .map { testingServer.bridge.renderFilter(it, table, context()) }
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query = "SELECT test_string FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
    ...
    fun testBooleanFiltersEquals() {
        ...
        val generatedFilter = test.filter(table)
            .map { testingServer.bridge.renderFilter(it, table, context()) }
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query = "SELECT ${castToDouble("count(*)")} FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
    ...
    fun testNumberFiltersEqualsInt() {
        ...
        val generatedFilter = test.filter(table)
            .map { testingServer.bridge.renderFilter(it, table, context()) }
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query = "SELECT ${castToDouble("test_int")} FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
    ...
    fun testNumberFiltersEqualsDouble() {
        ...
        val generatedFilter = test.filter(table)
            .map { testingServer.bridge.renderFilter(it, table, context()) }
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query = "SELECT ${castToDouble("test_double")} FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
    ...
    fun testNumberFiltersGreaterThanInt() {
        ...
        val generatedFilter = test.filter(table)
            .map { testingServer.bridge.renderFilter(it, table, context()) }
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query = "SELECT ${castToDouble("test_int")} FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter ORDER BY test_int"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
    ...
    fun testNumberFiltersGreaterThanDouble() {
        ...
        val generatedFilter = test.filter(table)
            ...
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query =
            "SELECT ${castToDouble("test_double")} FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter ORDER BY test_double"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
    ...
    fun testTimestampGreaterThan() {
        ...
        val generatedFilter = test.filter(table)
            ...
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query =
            "SELECT test_timestamp FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter"
    ...
    fun testDateGreaterThan() {
        ...
        val generatedFilter = test.filter(table)
            .map { testingServer.bridge.renderFilter(it, table, context()) }
            .joinToString(" AND ") { "(${it.whereFilter})" }
        val query = "SELECT test_date FROM ${testingServer.bridge.quoteIdentifier(table)} WHERE $generatedFilter"
        assertEquals(test.result, testingServer.runQueryFirstRow(query))
    }
